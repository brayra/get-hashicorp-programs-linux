...
usage: $PROG [Options] [VERSION]
   ...
   Download and install a version of vault
...
VERSIONS=$(wget -O - -q https://releases.hashicorp.com/vault/|grep 'href="/vault/'|egrep -v '(rc|oci|alpha|beta)'|grep -v '[+-]'|sed 's!^.*vault/\([^/]*\).*$!\1!g'|head -n +20)
...
tmp_dir=$(mktemp -d -t vault-XXXXXXXXXX)
DEST="${tmp_dir}/vault_${VERSION}_linux_amd64.zip"
